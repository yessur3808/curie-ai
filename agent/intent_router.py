...
TOOL_PARAMETER_SPECS: dict[str, ParameterSpec] = {
    ...
    "gmail_search": ParameterSpec(
        required=("query",), validators=(("query", _NONEMPTY),)
    ),
    "gmail_read": ParameterSpec(
        required=("message_id",), validators=(("message_id", _NONEMPTY),)
    ),
    "gmail_send": ParameterSpec(
    ...
    "x_read": ParameterSpec(
        required=("post_id",),
        validators=(
            ("post_id", lambda value: isinstance(value, str) and value.isdigit()),
        ),
    ),
    "x_post": ParameterSpec(required=("text",), validators=(("text", _NONEMPTY),)),
    "x_reply": ParameterSpec(
    ...
    "browser_click": ParameterSpec(
        required=("text",), validators=(("text", _NONEMPTY),)
    ),
    "browser_fill": ParameterSpec(
        required=("label", "value"),
        validators=(("label", _NONEMPTY), ("value", _NONEMPTY)),
    ),
    ...
    "inspect_project": ParameterSpec(
        defaults=(("path", "."),), validators=(("path", _PATH),)
    ),
    "create_directory": ParameterSpec(
        required=("name",),
        validators=(
            (
                "name",
                lambda value: isinstance(value, str) and bool(_NAME.fullmatch(value)),
            ),
        ),
    ),
    "create_python_project": ParameterSpec(
        required=("name",),
        validators=(
            (
                "name",
    ...
    "run_tests": ParameterSpec(
        defaults=(("path", "."),), validators=(("path", _PATH),)
    ),
    "project_change": ParameterSpec(
        required=("request",),
        defaults=(("path", "."), ("run_tests", False)),
    ...
    "home_control": ParameterSpec(
        required=("state",),
        defaults=(("target", ""), ("targets", ()), ("provider", "")),
        validators=(
            ("target", lambda value: isinstance(value, str)),
            ("targets", lambda value: value in ((), []) or _TARGETS(value)),
            ("state", _POWER_STATE),
            ("provider", lambda value: isinstance(value, str)),
        ),
    ),
...
def validate_tool_request(
    ...
    allowed = (
        set(spec.required)
        | {key for key, _ in spec.defaults}
    ...
    for key, validator in spec.validators:
        if key in normalized and not validator(normalized[key]):
            return (
                None,
                f"The {key.replace('_', ' ')} is not valid. Could you rephrase it?",
            )
...
def _home_target(value: str) -> str:
    ...
    named = re.search(
        r"\b(?:which\s+is\s+)?(?:called|named)\s+(?:the\s+)?(.+)$", target, re.I
    )
    if named:
...
def _home_control_request(command: str) -> ToolRequest | None:
    ...
    for pattern in patterns:
        ...
        if _pronoun_kind(target):
            return ToolRequest(
                "clarify",
                {"message": "Which home device should I control? Please use its name."},
            )
...
def _corrective_home_request(
    command: str, history: Iterable[Any] | None
) -> ToolRequest | None:
    """Retry a recent device goal when the user reports the opposite state."""
    report = re.fullmatch(
        r"(?:the\s+)?(?P<target>.+?)\s+(?:is|stayed|remains?)\s+"
        r"(?:still\s+)?(?P<state>on|off)[.!?]?",
        command.strip(),
        re.I | re.S,
    )
    if not report:
        return None
    reported_state = report.group("state").casefold()
    for item in reversed(list(history or [])[-8:]):
        if isinstance(item, Mapping):
            role, content = item.get("role"), item.get("content")
        elif isinstance(item, (tuple, list)) and len(item) >= 2:
        ...
        if str(role).casefold() != "user" or not isinstance(content, str):
            continue
        previous = classify_request(content)
        if not previous or previous.action != "home_control":
            continue
        requested_state = str(previous.params.get("state") or "")
        if requested_state and requested_state != reported_state:
            return ToolRequest(
                "home_control",
                dict(previous.params),
                explanation="retry the active smart-home goal after a contradictory state report",
            )
        return None
    return None
...
def classify_request(
    ...
    if match:
        return ToolRequest(
            "home_status",
            {"target": _home_target(match.group(1) or ""), "provider": ""},
        )
    ...
    if match:
        ...
        return (
            ...
            if error
                ...
                "home_control",
                params or {},
                explanation=f"turn {target} {match.group(1).casefold()}",
            )
        )
    ...
    if match and re.search(
        r"\b(?:device|light|lamp|plug|switch|fan|air|ac|tapo|nanoleaf|smartthings|petlibro|govee|mi home|xiaomi|thinq)\b",
        match.group(1),
        re.I,
    ):
        return ToolRequest(
    ...
    if match:
        ...
        if target.casefold() not in conversational and len(target.split()) <= 8:
    ...
    if match:
        ...
        return ToolRequest(
            ...
            {"text": label},
            needs_approval=True,
            explanation=f"click the browser element labelled {label!r}",
        )
    ...
    if match:
        ...
        return ToolRequest(
            ...
            needs_approval=True,
            explanation=f"fill the browser field labelled {label!r}",
        )
    ...
    match = re.fullmatch(
        r"/gmail\s+send\s+([^|]+)\|([^|]+)\|(.+)", command, re.I | re.S
    )
    if match:
        ...
        return ToolRequest(
            ...
            needs_approval=True,
            explanation=f"send this email to {recipient}",
        )
    ...
    if match:
        return ToolRequest(
            ...
            needs_approval=True,
            explanation="publish this post on X",
        )
    match = re.fullmatch(
        r"/(?:x|twitter)\s+reply\s+(\d+)\s*\|\s*(.+)", command, re.I | re.S
    )
    if match:
        return ToolRequest(
            "x_reply",
            {"post_id": match.group(1), "text": match.group(2).strip()},
            needs_approval=True,
            explanation=f"publish this reply to X post {match.group(1)}",
        )
    if re.fullmatch(r"/(?:x|twitter)\s+dm\s+(?:read|list)", command, re.I):
        return ToolRequest("x_dm_read")
    match = re.fullmatch(
        r"/(?:x|twitter)\s+dm\s+send\s+(\d+)\s*\|\s*(.+)", command, re.I | re.S
    )
    if match:
        return ToolRequest(
            "x_dm_send",
            {"participant_id": match.group(1), "text": match.group(2).strip()},
            needs_approval=True,
            explanation=f"send this X direct message to user ID {match.group(1)}",
        )
    ...
    if re.search(
        r"\b(?:search|find|look for)\b.{0,30}\b(?:gmail|emails?|inbox)\b|\b(?:gmail|inbox)\b.{0,30}\b(?:search|find)\b",
        lowered,
    ):
        candidates.append(ToolRequest("gmail_search", {"query": text}))
    if re.search(
        r"\b(?:search|find)\b.{0,30}\b(?:x|twitter|tweets?|posts?)\b|\b(?:x|twitter)\b.{0,30}\b(?:search|find)\b",
        lowered,
    ):
        candidates.append(ToolRequest("x_search", {"query": text}))
    if re.search(
        r"\b(?:read|show|check)\b.{0,30}\b(?:x|twitter)\s+(?:dms?|direct messages?)\b",
        lowered,
    ):
        candidates.append(ToolRequest("x_dm_read"))
    if re.search(
        r"\b(?:create|start|make|scaffold)\b.*\b(?:python\s+)?project\b", lowered
    ):
        ...
        if name:
            ...
            return ToolRequest(
                "clarify", {"message": "What should I name the Python project?"}
            )
    if re.search(r"\b(?:create|make)\b.*\b(?:directory|folder)\b", lowered):
        ...
        if name:
            ...
            return ToolRequest(
                "clarify", {"message": "What should I name the directory?"}
            )
    if re.search(
        r"\b(?:fix|implement|add|generate|modify|adjust|refactor)\b.*\b(?:bug|code|endpoint|project|feature|tests?)\b",
        lowered,
    ):
        candidates.append(
            ToolRequest(
                "project_change",
                {
                    ...
                    "path": ".",
                    "run_tests": bool(
                        re.search(r"\b(?:run|execute)\b.*\btests?\b", lowered)
                    ),
                },
                needs_approval=True,
        ...
        r"\b(?:look through|inspect|list|show|review)\b.*\b(?:files?|project|repository|repo)\b",
        lowered,
    ):
    ...
    if not re.search(
        r"\b(?:weather|storm|rain)\s+(?:metaphor|symbol|imagery)\b", lowered
    ) and re.search(
    ...
    if len(unique) == 1:
        ...
        normalized, clarification = validate_tool_request(
            request.action, request.params
        )
        return (
            ToolRequest("clarify", {"message": clarification})
            if clarification
            else ToolRequest(
                request.action,
                normalized or {},
    ...
    if len(unique) > 1:
        ...
        return ToolRequest(
            "clarify",
            {
                "message": f"I found more than one possible action: {actions}. Which one should I do first?"
            },
        )
    return None
