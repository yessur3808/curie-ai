...
def create_or_update_pr(
    ...
    if pulls.totalCount == 0:
        pr = gh_repo.create_pull(
            title=pr_title, body=pr_body, head=branch_name, base=target_base
        )
...
def apply_code_change(goal, files_to_edit, repo_path, branch_name):
    ...
    suggestions_md = ai_project_idea_suggestions(
        goal, files_to_edit, repo_path, CODING_MODEL_NAME
    )
...
def get_coding_model_name():
    ...
    if coding_model and coding_model not in llm_models:
        print(
            f"Warning: CODING_MODEL_NAME '{coding_model}' is not found in LLM_MODELS ({llm_models})."
        )
        ...
        if llm_models:
            ...
            try:
                choice = input(
                    "Select a model by number (or press Enter to abort): "
                ).strip()
