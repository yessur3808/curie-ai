...
class GitHubIntegration:
    ...
    def git_status(self) -> Dict[str, List[str]]:
        ...
        status = {"staged": staged, "unstaged": changed, "untracked": list(untracked)}

        logger.info(
            f"Git status: {len(staged)} staged, {len(changed)} unstaged, {len(untracked)} untracked"
        )
        return status
    ...
    def list_pull_requests(
        self, state: str = "open", repo_name: Optional[str] = None
    ) -> List[Dict]:
        """
        List pull requests
...
def quick_commit_and_push(
    files: List[str], message: str, branch: Optional[str] = None, repo_path: str = "."
) -> bool:
    """
