...
class BugDetector:
    ...
    def detect_bugs_in_code(
        self, code: str, language: Optional[str] = None, filepath: Optional[str] = None
    ) -> Dict:
        """
    ...
    def format_findings_report(self, results: Dict) -> str:
        ...
        for severity in ["critical", "high", "medium", "low"]:
            if severity in findings_by_severity:
                ...
                if len(findings) > 5:
                    report += (
                        f"*... and {len(findings) - 5} more {severity} issues*\n\n"
                    )
    ...
    def format_proactive_scan_report(self, results: Dict) -> str:
        ...
        if files_with_issues:
            ...
            sorted_files = sorted(
                files_with_issues, key=lambda x: x["total_findings"], reverse=True
            )
...
def detect_bugs(
    code: str, language: Optional[str] = None, filepath: Optional[str] = None
) -> Dict:
    """
