...
class GitLabIntegration:
    ...
    def __init__(self, token: Optional[str] = None, base_url: Optional[str] = None):
        ...
        self.base_url = (
            base_url or os.getenv("GITLAB_URL", "https://gitlab.com")
        ).rstrip("/")
    ...
    def create_merge_request(
        ...
        response = requests.post(
            url, headers=self.headers, json=data, timeout=HTTP_TIMEOUT
        )
        response.raise_for_status()
    ...
    def review_merge_request(
        self, project_path: str, mr_iid: int, post_comment: bool = True
    ) -> Dict:
        """
    ...
    def create_and_review_mr(
        ...
        review_result = self.review_merge_request(
            project_path, mr_data["iid"], post_comment=True
        )
...
def apply_gitlab_code_change(
    ...
    mr_data, review_result = gitlab.create_and_review_mr(
        repo_path, MAIN_REPO, branch_name, target_base, pr_title, mr_body
    )
