...
async def execute_request(req: ToolRequest, internal_id: str, profile: dict, *, approved: bool = False) -> str:
    ...
    if req.needs_approval:
        ...
        _audit(internal_id, req.action, "pending", parameters=safe_params, policy_decision="approval_required", approval={"required": True, "granted": False, "token": token}, connector=profile.get("_connector", "unknown"))
        return (
    ...
    try:
        ...
        safe_params = {
            key: "[REDACTED]" if key in getattr(definition, "audit_redactions", ()) else value
            for key, value in req.params.items()
        }
        ...
        _audit(
            ...
            validated_action=req.action, parameters=safe_params, policy_decision="allowed",
            ...
            command_exit_status=tool_result.data.get("exit_status") if "command" in tool_result.data else None, citations=citations,
        ...
        safe_params = {key: "[REDACTED]" if key in redactions else value for key, value in req.params.items()}
        _audit(internal_id, req.action, "failed", connector=profile.get("_connector", "unknown"), validated_action=req.action, parameters=safe_params, policy_decision="execution_failed", approval={"required": req.needs_approval, "granted": approved}, finished_at=datetime.now(timezone.utc), outcome={"status": "failed", "error": str(exc)}, security_category="tool_failure")
