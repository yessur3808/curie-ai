...
class ScraperPatternManager:
        ...
        with get_pg_conn() as conn:
            ...
            """,
                (
                    ...
                    domain,
                    query_type,
                    Json(content_pattern) if content_pattern is not None else None,
                    last_success,
                    last_error,
                    reliability_score,
                    created_at,
    ...
    def load_by_domain(domain, limit=10):
        with get_pg_conn() as conn:
            ...
            return [
                dict(zip([desc[0] for desc in cur.description], row)) for row in results
            ]
    ...
    def update_pattern(id, **fields):
        ...
        allowed_fields = {
            k: v for k, v in fields.items() if k in ALLOWED_UPDATE_COLUMNS
        }
        if not allowed_fields:
            return False
        # Wrap content_pattern with Json() if present
        if (
            "content_pattern" in allowed_fields
            and allowed_fields["content_pattern"] is not None
        ):
