...
Endpoints are `GET /health`, `POST /chat`, `POST /speak`, `POST /speak-stream`, `POST /transcribe`, and `GET /audio/{name}`. `/speak-stream` accepts `{"text":"Bonjour. I am here.","voice_profile":"custom"}` and returns newline-delimited `ready`, `audio`, and `done`/`error` events. Audio URLs are private to the same service. Keep this API on loopback behind the dashboard; it is not an independently authenticated public server.
...
Health checks read metadata and never import the speech model. Only one speech/chat operation runs at a time. The worker uses six CPU threads at reduced priority, offline model access, bounded text and incremental audio output, then exits. The bridge kills it after 115 seconds. Report line breaks are preserved as sentence boundaries, and visual separators become spoken pauses. Invalid or unusually long chunks get one bounded retry with the same text and trained voice; persistent failures are rejected rather than serving likely repetition. A failed generation leaves text available and never silently switches voices.
...
To roll back, atomically restore `data/trained-voice/config-v1.json` as `config.json`, then restart only PM2 process `curie-dashboard-api`. The old profile and original base weights remain intact. To disable custom synthesis, set `enabled` false and choose a Piper preset. No existing Curie bot channels or their voice settings are modified.
