...
from services.audit import handle_audit_command, normalize_audit_details, redact, security_alerts


def test_recursive_redaction_hides_secrets_contents_and_sensitive_paths(tmp_path, monkeypatch):
...
def test_normalized_event_has_reconstruction_fields_without_content():
    event = normalize_audit_details({
        "connector": "telegram", "validated_action": "research",
        "params": {"query": "my private plans", "api_key": "secret"},
        "policy_decision": "allowed", "approval": {"required": False},
        "tool_version": "2.1", "result": "personal response",
...
def test_sqlite_audit_is_chained_private_exportable_and_owner_deletable(tmp_path, monkeypatch):
    ...
    repo.append("u1", "change", "completed", {"changed_files": [tmp_path / "secret.txt"]})
    repo.append("u2", "inspect", "completed", {})
    events = repo.list("u1", 10)
    assert events[0]["details"]["previous_hash"] == events[1]["details"]["integrity_hash"]
