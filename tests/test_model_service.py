...
def test_explicit_depth_keeps_a_large_generation_budget():
    ...
    assert (
        ModelConversationService.token_budget("Give me a thorough comparison") == 1024
    )
...
def test_task_content_selects_specialized_model_roles():
    assert (
        ModelConversationService.model_role("Fix the authentication bug and run pytest")
        == "coding"
    )
    assert (
        ModelConversationService.model_role("Analyze these market tradeoffs")
        == "reasoning"
    )
    ...
    assert (
        ModelConversationService.model_role("Correlate these symptoms") == "reasoning"
    )
    assert (
        ModelConversationService.model_role("Recommend the best option") == "reasoning"
    )
...
def test_local_fallback_receives_selected_role(monkeypatch):
    ...
    result = asyncio.run(
        ModelConversationService(local).generate(
            "prompt", "debug this Python code", 0.2
        )
    )
