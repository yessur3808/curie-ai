...
async def test_search_sources_llm_returns_safe_urls():
    ...
    with (
        patch("agent.skills.find_info.manager.ask_llm") as mock_ask_llm,
...
async def test_search_sources_llm_with_all_unsafe_urls():
    ...
    with (
        ...
        mock_ask_llm.return_value = "http://localhost:8080\n" "http://127.0.0.1\n"
...
async def test_adaptive_scraper_analyze_webpage():
    ...
    with (
        patch("agent.skills.find_info.load_scraper_pattern") as mock_load,
        ...
        mock_scrape.assert_called_once_with(
            "https://example.com", pattern={"main_selector": ".content"}
        )
...
async def test_find_info_success():
    ...
    with (
        patch("agent.skills.find_info.DynamicScraper") as mock_scraper_class,
        ...
        assert (
            mock_adaptive.save_scraper_pattern.call_count == 2
        )  # Pattern saved for each URL
