...
def test_dedupe_cache_concurrent_same_key():
    ...
    assert (
        false_count == 1
    ), f"Exactly one thread should see the key as new, got {false_count}"
    assert (
        true_count == 19
    ), f"19 threads should see the key as duplicate, got {true_count}"
...
def test_dedupe_cache_expired_key_fifo_order():
    ...
    assert (
        cache.check("key1", now + 101) is False
    ), "Expired key1 should be treated as new"
    assert cache.size() == 3
    ...
    assert (
        cache.check("key3", now + 103) is True
    ), "key3 should still be in cache (93s old)"
    assert (
        cache.check("key1", now + 103) is True
    ), "key1 (re-inserted) should still be in cache (2s old)"
    assert (
        cache.check("key4", now + 103) is True
    ), "key4 should still be in cache (1s old)"

    # key2 should have been evicted
    assert (
        cache.check("key2", now + 103) is False
    ), "key2 should have been evicted (treated as new)"
