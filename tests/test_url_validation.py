...
async def test_block_private_ips():
    ...
    assert (
        await is_safe_url("http://10.0.0.1:8080") == False
    )  # Port 8080 blocked on private IP

    # 172.16.0.0/12
    assert await is_safe_url("http://172.16.0.1") == False
    assert await is_safe_url("http://172.31.255.255") == False
    assert (
        await is_safe_url("http://172.16.0.1:8080") == False
    )  # Port 8080 blocked on private IP
    ...
    assert (
        await is_safe_url("http://192.168.1.1:8080") == False
    )  # Port 8080 blocked on private IP
...
async def test_block_link_local():
    ...
    assert (
        await is_safe_url("http://169.254.169.254:8080") == False
    )  # Port 8080 blocked on link-local IP
...
async def run_all_tests():
    ...
    print(
        "are not included because they require DNS resolution, which is not available"
    )
