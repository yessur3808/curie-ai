...
def main():
    ...
    if is_port_in_use(5432):
        ...
        if pg_process:
            ...
            answer = (
                input(
                    "Do you want to try to stop the system PostgreSQL service? [y/N]: "
                )
                .strip()
                .lower()
            )
            if answer == "y":
                if not stop_postgres_service():
                    print(
                        "Could not stop system Postgres. You may need to manually stop it or change the Docker port."
                    )
                    sys.exit(1)
                else:
                ...
                print(
                    "You can change the port in your docker-compose.yml (e.g., 5433:5432) and update your app config."
                )
                sys.exit(1)
        else:
            print(
                "Port 5432 is in use, but not by a 'postgres' process. Investigate further!"
            )
            sys.exit(1)
    else:
