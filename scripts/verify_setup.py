...
def check_python_version() -> bool:
    ...
    if version.major == 3 and version.minor >= 10:
        ...
        print(
            f"{RED}✗{RESET} Python version {version.major}.{version.minor}.{version.micro} is too old. Required: 3.10 or higher"
        )
        return False
...
def check_env_file() -> bool:
    ...
    if os.path.exists(".env"):
        ...
        print(
            f"{YELLOW}⚠{RESET} .env file not found (you may need to copy .env.example)"
        )
        return False
...
def check_database_config() -> Tuple[bool, List[str]]:
    ...
    if missing:
        print(
            f"{YELLOW}⚠{RESET} Database config incomplete. Missing: {', '.join(missing)}"
        )
...
def main():
    ...
    try:
        ...
        print(
            f"{YELLOW}⚠ python-dotenv is not installed; skipping .env loading.{RESET}"
        )
    else:
        try:
            load_dotenv()
    ...
    if all_passed and not optional_missing:
        ...
        print(
            f"{YELLOW}⚠ Core setup is complete, but some optional features are unavailable.{RESET}\n"
        )
        if optional_missing:
