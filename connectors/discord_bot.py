...
def get_internal_id(
    discord_user_id: int, discord_username: str, platform: str = "discord"
) -> str:
    """
...
async def handle_voice_attachment(attachment, persona) -> Optional[str]:
    ...
    try:
        ...
        transcribed_text = await transcribe_audio(
            audio_path, language=language, accent=accent, auto_detect=True
        )
...
# Only define DiscordBot class if discord.py is available
if commands is not None:
    ...
    class DiscordBot(commands.Bot):
        ...
        async def on_ready(self):
            ...
            logger.info(
                f"🤖 Discord bot logged in as {self.user.name} (ID: {self.user.id})"
            )
    ...
    def create_discord_bot(workflow: ChatWorkflow) -> "DiscordBot":
        ...
        async def resume_command(ctx):
            ...
            await ctx.send("Bienvenue! I'm here and ready to chat again. 😊")
        ...
        async def identify_command(ctx, secret_username: str = None):
            ...
            internal_id = UserManager.get_internal_id_by_secret_username(
                secret_username
            )
            if internal_id:
    ...
    **Available Commands:**
    ...
    def start_discord_bot(workflow: ChatWorkflow):
        ...
        if not discord_token:
            raise RuntimeError(
                "Discord bot token not found in .env file or environment variables."
            )
    ...
    def create_discord_bot(workflow):
        """Stub function when discord.py is not available."""
        raise RuntimeError(
            "discord.py is not installed. Install it with: pip install discord.py"
        )

    def start_discord_bot(workflow):
        """Stub function when discord.py is not available."""
        raise RuntimeError(
            "discord.py is not installed. Install it with: pip install discord.py"
        )
