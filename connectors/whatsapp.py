...
def get_internal_id(
    wa_user_id: str, wa_username: str, platform: str = "whatsapp"
) -> str:
    """
...
async def handle_voice_message(message) -> Optional[str]:
    ...
    try:
        ...
        transcribed_text = await transcribe_audio(
            audio_file, language=language, accent=accent, auto_detect=True
        )
...
async def handle_message(message):
    ...
    try:
        ...
        if user_message.startswith("/resume"):
            ...
            await message.reply("Bienvenue! I'm here and ready to chat again. 😊")
            return
        ...
        if user_message.startswith("/identify"):
            ...
            internal_id_found = UserManager.get_internal_id_by_secret_username(
                secret_username
            )
